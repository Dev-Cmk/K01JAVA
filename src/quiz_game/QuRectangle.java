...
class Rectangle
{
	//멤버변수
		...
		*/
   		...
   		if(this.ulx < 0 || this.ulx > 100) {
   			System.out.println("좌상단좌표 범위가 잘못되었다.");
   		}
   		if(this.uly < 0 || this.uly > 100) {
   			System.out.println("우하단 좌표 범위가 잘못되었다.");
   		}
   		if(this.ulx > this.lrx || this.uly > this.lry) {
   	...
   	{
   		if(this.ulx > 0 && this.uly < 100 && this.uly > 0 && this.uly < 100
   				&& this.ulx < this.lrx && this.uly <this.lry) {
   			/* 사각형의 넓이를 구하는 메소드 정의 */
   	   		int area;
   	   		area = (this.lrx-this.ulx)*(this.lry-this.uly);
   	   		System.out.println("넓이:"+area);
   		}
   		else {
   			System.out.println("좌표값 오류로 넚이름 계산할수 없습니다.");
   		}
   	}  	
}
